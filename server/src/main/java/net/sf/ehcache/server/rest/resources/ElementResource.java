 ...
 *
 * @author Greg Luck
 * @author Yaroslav Mykhaylov !!! Fixed problems with decorator settings for GET
 * requests
 */
public class ElementResource {
    ...
    public Response getElement() {
        ...
        net.sf.ehcache.Ehcache ehcache = lookupEhcache();
        net.sf.ehcache.Element ehcacheElement = lookupElement(ehcache);
        Element localElement = new Element(ehcacheElement, uriInfo.getAbsolutePath().toString());
     ...
     *
     * @param headers this method parses the mime type header and also
     * ehcacheTimeToLiveSeconds, which is used to override the cache's default
     * time to live.
...
//            URI cacheUri = uriInfo.getAbsolutePathBuilder().path("..").build().normalize();
     ...
     * <p/>
     * If the element resource is specified as "*", then
     * <code>cache.removeAll()</code> is called.
     *
    ...
    /**
     * Each time an element is put into ehcache the creation time is set even if
     * it is an update. So, "creation time" means Last-Modified.
     *
     * @param ehcacheElement the underlying Ehcache element
     * @return the last modified date. If this is the first version of the
     * element, the last-modified means the name things as created. This date is
     * accurate to ms, however the HTTP protocol is not - it only goes down to
     * seconds. Jersey removes the ms.
     */
    ...
    /**
     * A very performant ETag implementation. This will be unique across JVM
     * restarts, or deleting an element and putting one back in.
     *
     * @param ehcacheElement A backing ehcache element
     * @return the ETag for this entry
     * @see <a
     * href="http://www.w3.org/Protocols/rfc2616/rfc2616-sec3.html#sec3.11">HTTP/1.1
     * section 3.11</a>
     */
    ...
    private net.sf.ehcache.Element lookupElement(net.sf.ehcache.Ehcache ehcache) throws WebApplicationException {
        net.sf.ehcache.Element ehcacheElement = ehcache.get(element);
        if (ehcacheElement == null) {
            throw new WebApplicationException(Response.Status.NOT_FOUND);
        }
    ...
    /**
     * Looks up the cache in the instance field
     * <code>cache</code>
     *
     * @return An ehcache element. This method will not return null.
     */
    ...
    private net.sf.ehcache.Ehcache lookupEhcache() throws WebApplicationException {
        net.sf.ehcache.Ehcache ehcache = MANAGER.getEhcache(cache);
        if (ehcache == null) {
            throw new WebApplicationException(Response.Status.NOT_FOUND);
        }
        return ehcache;
